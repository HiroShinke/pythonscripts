...
"""
    ...
    return transposeTable(tmptable)
...
def printAsArray(table):
    ret = []
    for i in range(0,32):
        for j in range(0,8):
            x = table[i*8 + j]
            ret.append(f"0x{x:02x}, ")
        ret.append("\n")
    return "[" + ("".join(ret)) + "]"
...
def print_decoding_table(table):
    for i in table:
        print(make_decode_str(i))
        
def make_decode_str(x):
    try:
        chars = str(bytes((x,)),"cp932")
        if chars.isprintable():
            return "'" + chars + "'"
...
def main():
    ...
    print("EBCDIK")
    print(printAsArray(ebcdik2ascii))

    print_decoding_table(ebcdik2ascii)
