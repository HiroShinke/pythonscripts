...
class SimpleTest(unittest.TestCase):
    ...
    def test3(self):
        ...
        expr.defined(seqs(term, manys( seqs(addop, term)) ))
        term.defined(seqs(atom, manys( seqs(mulop, atom)) ))
        atom.defined(Or(digit,
                        Seq( rep(r"\("),
                             expr,
                             rep(r"\)") )))
    ...
    def test4(self):
        ...
        condexpr.defined(Seq(condterm, Many( Seq(logicalop, condterm) )))
        condterm.defined(Or(binexp = Seq(lhs = expr, binop = bincondop, rhs = expr),
                            unaryexp = Seq(expr, unarycondop),
                            simpleexp = expr,
                            bracedexp = Seq(rep("\("), condexpr, rep("\)"))))

        condexpr.setTag("condexpr")
        condterm.setTag("condterm")
